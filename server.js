...
async function getLoginSessionInfo() {
  ...
  const loginCookies = (storageState.cookies || []).filter((cookie) => LOGIN_COOKIE_NAMES.has(cookie.name));
  ...
  return {
    loginAvailable: true,
    loginValid: activeCookies.length > 0,
    needsLogin: activeCookies.length === 0,
    accountLabel: LOGIN_LABEL || '로그인 세션',
    expiresAt: expiresAt ? new Date(expiresAt * 1000).toISOString() : '',
    reason: activeCookies.length > 0 ? '' : '신라 로그인 토큰이 없거나 만료되었습니다.',
  };
}
