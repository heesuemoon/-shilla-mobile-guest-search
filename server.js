...
async function gotoAndContinue(page, url, { timeout = 45000 } = {}) {
  try {
...
async function scrapeShillaInContext(context, keyword, maxResults = DEFAULT_MAX_RESULTS) {
  ...
  try {
    ...
    await gotoAndContinue(page, searchUrl, { timeout: 45000 });
    ...
    const extracted = await page.evaluate(
      ({ maxResults: limit, origin }) => {
        ...
        for (const card of cards) {
          ...
          const productSku = skuFromText(`${imageSrc} ${href} ${card.innerHTML}`);
...
async function scrapeProductBenefits(context, item, { inputQuery, inputSku, loginApplied }) {
  ...
  try {
    page.setDefaultTimeout(30000);
    await gotoAndContinue(page, productUrl, { timeout: 45000 });
    ...
    const detail = await page.evaluate(
      ({ fallback }) => {
        ...
        const productSku =
          ...
          skuFromText(`${firstAttr(['img[src]'], 'src')} ${document.body.innerHTML}`) ||
