...
async function scrapeProductBenefits(context, item, { inputQuery, inputSku, loginApplied }) {
  ...
  const fallbackSku = inputSku || extractSku(item.productName) || item.productCode || '';
  ...
  if (!productUrl) {
    return {
      ...
      shillaRewardRate: '',
      shillaSPoint: '',
      benefitBasis: loginApplied ? '로그인' : '비회원',
      benefitText: '',
      productUrl: '',
    };
  }
  ...
  try {
    ...
    return {
      sourceQuery: inputQuery,
      brandCode: brandCodeFromSku(inputSku || productSku),
...
async function scrapeBenefitRows(keyword, maxResults = DEFAULT_BENEFIT_MAX_RESULTS) {
  ...
  try {
    const search = await scrapeShillaInContext(context, keyword, maxResults);
