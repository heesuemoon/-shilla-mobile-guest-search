...
const DEFAULT_BENEFIT_MAX_RESULTS = 100;
...
const BRAND_EXACT_FILTERS = new Map([
  ['조니워커', ['조니워커', 'JOHNNIE WALKER', 'JOHNNIE']],
  ['조니 워커', ['조니워커', 'JOHNNIE WALKER', 'JOHNNIE']],
  ['johnnie walker', ['조니워커', 'JOHNNIE WALKER', 'JOHNNIE']],
  ['헤네시', ['헤네시', 'HENNESSY']],
  ['hennessy', ['헤네시', 'HENNESSY']],
  ['앱솔루트', ['앱솔루트', 'ABSOLUT']],
  ['absolut', ['앱솔루트', 'ABSOLUT']],
  ['발렌타인', ['발렌타인', "BALLANTINE'S", 'BALLANTINE']],
  ['ballantine', ['발렌타인', "BALLANTINE'S", 'BALLANTINE']],
  ['ballantines', ['발렌타인', "BALLANTINE'S", 'BALLANTINE']],
  ['키엘', ['키엘', "KIEHL'S", 'KIEHLS', 'KIEHL']],
  ['kiehls', ['키엘', "KIEHL'S", 'KIEHLS', 'KIEHL']],
  ['kiehl', ['키엘', "KIEHL'S", 'KIEHLS', 'KIEHL']],
  ['헤라', ['헤라', 'HERA']],
  ['hera', ['헤라', 'HERA']],
  ['설화수', ['설화수', 'SULWHASOO']],
...
function brandFilterForInput(originalQuery) {
  const key = normalizeBrandKey(originalQuery);
  if (!key || /^\d+$/.test(key) || /^https?:/i.test(String(originalQuery || ''))) return [];

  for (const [inputKey, brandNames] of BRAND_EXACT_FILTERS) {
    if (normalizeBrandKey(inputKey) === key) {
      return brandNames.map(normalizeBrandKey).filter(Boolean);
    }
  }
...
function itemMatchesBrandFilter(item, brandFilters) {
  if (!brandFilters.length) return true;

  const brand = normalizeBrandKey(item.brand);
  if (brand) {
    return brandFilters.some((filter) => brand === filter || brand.includes(filter) || filter.includes(brand));
  }

  const productName = normalizeBrandKey(item.productName);
  return productName ? brandFilters.some((filter) => productName.includes(filter)) : false;
}

function filterSearchItemsForInput(items, input) {
  const brandFilters = brandFilterForInput(input.originalQuery);
  if (!brandFilters.length) return items;
  return items.filter((item) => itemMatchesBrandFilter(item, brandFilters));
}
...
async function createMobileContext(browser, { useLogin = false } = {}) {
  if (useLogin && USE_LOGIN_PROFILE) {
    const persistentContext = await getLoginPersistentContext().catch((error) => {
...
function dollarValueOnly(value) {
  const text = cleanText(value);
  if (!text) return '';

  const explicitDollar = text.match(/\$\s*([0-9][\d,]*(?:\.\d+)?)/);
  if (explicitDollar?.[1]) return plainNumber(explicitDollar[1]);

  const usdText = text.match(/USD\s*([0-9][\d,]*(?:\.\d+)?)/i);
  if (usdText?.[1]) return plainNumber(usdText[1]);

  if (/원|만원|%|P|포인트/i.test(text)) return '';
  const numericOnly = text.match(/^-?[0-9][\d,]*(?:\.\d+)?$/);
  return numericOnly ? plainNumber(numericOnly[0]) : '';
}

function percentValueOnly(value) {
  const text = cleanText(value);
  if (!text) return '';

  const explicitPercent = text.match(/(-?[0-9][\d,]*(?:\.\d+)?)\s*%/);
  if (explicitPercent?.[1]) return plainNumber(explicitPercent[1]);
...
function krwValueOnly(value) {
  const text = cleanText(value);
  if (!text) return '';

  const manWon = text.match(/([0-9][\d,]*(?:\.\d+)?)\s*만원/);
  if (manWon?.[1]) return String(Math.round(Number(plainNumber(manWon[1])) * 10000));

  const won = text.match(/([0-9][\d,]*)\s*원/);
  if (won?.[1]) return plainNumber(won[1]);

  if (/\$|%|P|포인트/i.test(text)) return '';
  const numericOnly = text.match(/^-?[0-9][\d,]*(?:\.\d+)?$/);
  return numericOnly ? plainNumber(numericOnly[0]) : '';
}

function pointValueOnly(value) {
  const text = cleanText(value);
  if (!text) return '';

  const point = text.match(/([0-9][\d,]*(?:\.\d+)?)\s*(?:P|포인트)/i);
  if (point?.[1]) return plainNumber(point[1]);

  if (/\$|원|만원|%/i.test(text)) return '';
  const numericOnly = text.match(/^-?[0-9][\d,]*(?:\.\d+)?$/);
  return numericOnly ? plainNumber(numericOnly[0]) : '';
}

function normalizeBenefitValue(label, value) {
  const key = cleanText(label);
  const text = cleanText(value);
  if (!text) return { value: '', unit: '' };
  if (/쿠폰\s*다운로드|다운로드/i.test(key) || /로그인|실패|필요|없음|완료|받기/i.test(text)) {
    return { value: text, unit: '' };
  }

  const preferPercent = /할인|쿠폰|적립률|L\.?\s*POINT/i.test(key);
  const preferKrw = /토스|현대|카드|Apple\s*Pay|카카오페이|결제|원화/i.test(key);
  const preferPoint = /포인트|POINT|적립금|S포인트/i.test(key);

  const candidates = preferKrw
    ? [
        ['원화', krwValueOnly(text)],
        ['%', percentValueOnly(text)],
        ['$', dollarValueOnly(text)],
        ['P', pointValueOnly(text)],
      ]
    : preferPercent
      ? [
          ['%', percentValueOnly(text)],
  ...
  const matched = candidates.find(([, normalizedValue]) => normalizedValue);
  return matched ? { unit: matched[0], value: matched[1] } : { value: text, unit: '' };
}

function normalizeBenefitBreakdown(benefitBreakdown = {}, productDiscount = '') {
  const values = {};
  const units = {};

  for (const [label, rawValue] of Object.entries(benefitBreakdown || {})) {
    const key = cleanText(label);
    if (!key) continue;
    const normalized = normalizeBenefitValue(key, rawValue);
    if (!normalized.value) continue;
    values[key] = normalized.value;
    if (normalized.unit) units[key] = normalized.unit;
  }

  const normalizedDiscount = normalizeBenefitValue('상품 할인', productDiscount);
  if (normalizedDiscount.value && !values['상품 할인']) {
    values['상품 할인'] = normalizedDiscount.value;
    if (normalizedDiscount.unit) units['상품 할인'] = normalizedDiscount.unit;
  }

  return { values, units };
}

function normalizeCompareOutputRow(row = {}) {
  const productSku = normalizeSkuCandidate(row.productSku);
  const discountPriceKrw = krwValueOnly(row.discountPrice);
  const maxBenefitPriceKrw = krwValueOnly(row.maxBenefitPrice);
  const salePriceKrw = krwValueOnly(row.salePrice);
  const discountPrice = dollarValueOnly(row.discountPrice);
  const maxBenefitPrice = dollarValueOnly(row.maxBenefitPrice);
  const salePrice = dollarValueOnly(row.salePrice);
  const productDiscount = percentValueOnly(row.productDiscount || row.discountRate);
  const normalizedBenefits = normalizeBenefitBreakdown(row.benefitBreakdown || {}, productDiscount);

  return {
    ...row,
    brandCode: productSku ? brandCodeFromSku(productSku) : '',
    productSku,
    salePrice,
    salePriceKrw,
    discountPrice,
    ...
    maxBenefitPriceKrw,
    productDiscount,
    benefitBreakdown: normalizedBenefits.values,
    benefitUnits: normalizedBenefits.units,
  };
}

function isLoggedCompareRow(row = {}) {
  const basis = cleanText(row.benefitBasis);
  if (!basis) return false;
  if (/비로그인|비회원|로그인\s*필요|성인인증|보안|차단|실패|검색\s*목록|확인\s*실패/i.test(basis)) {
    return false;
  }
  return /로그인|회원|멤버십|GOLD|SILVER|BLACK|VIP|세션\s*사용/i.test(basis);
}
...
function normalizeFallbackItem(item = {}) {
  ...
  return {
    ...
    discountPrice: String(item.discountPrice || '').trim(),
    maxBenefitPrice: String(item.maxBenefitPrice || '').trim(),
...
function mergeBenefitRowWithFallback(row = {}, item = {}, { inputSku = '', loginApplied = false } = {}) {
  ...
  const merged = {
    ...row,
    brandCode: brandCodeFromSku(normalizedSku) || row.brandCode || item.brandCode || '',
    ...
    discountPrice: row.discountPrice || item.discountPrice || '',
    maxBenefitPrice: row.maxBenefitPrice || item.maxBenefitPrice || '',
    productDiscount: row.productDiscount || item.productDiscount || item.discountRate || '',
    benefitBreakdown:
      Object.keys(row.benefitBreakdown || {}).length > 0
        ? row.benefitBreakdown
        : item.productDiscount || item.discountRate
          ? { '상품 할인': item.productDiscount || item.discountRate }
          : {},
  };
...
async function getProductPageBlockState(page) {
  return page
    .evaluate(() => {
      const text = document.body?.innerText || '';
      const source = `${text} ${document.title || ''} ${document.documentElement?.innerHTML || ''}`;
      const hasProductDetail = /(최대\s*혜택가|상품\s*할인|장바구니|바로구매|상품정보|상품\s*상세|SKU|스큐)/i.test(text);
      const blockedBySecurity =
        /보안 확인|잠시만 기다리십시오|cloudflare|verify you are human|just a moment|enable javascript and cookies|challenge-platform|cf_chl/i.test(
          source,
        );
      const requiresAdultVerification =
        /(19\s*세|성인\s*인증|성인인증|연령\s*확인|본인\s*인증|본인인증|휴대폰\s*인증|주류\s*상품|미성년자)/i.test(text) &&
        !hasProductDetail;
      const requiresLogin =
        /(로그인\s*후|로그인이\s*필요|로그인하세요|로그인\s*필요|회원\s*로그인|로그인\s*해\s*주세요)/i.test(text) &&
        !/(로그아웃|마이페이지|회원등급|멤버십)/i.test(text) &&
        !hasProductDetail;
...
function blockedBenefitRow(item, { inputQuery, inputSku, loginApplied, productUrl, basis, details }) {
  const fallbackSku = inputSku || (isRealSku(item.productSku) ? item.productSku : '') || extractSku(item.productName) || '';
  return {
    sourceQuery: inputQuery,
    brandCode: brandCodeFromSku(fallbackSku) || item.brandCode || '',
    brand: item.brand || '',
    ...
    productDiscount: item.discountRate || item.productDiscount || '',
    dailyCoupon: '',
    rewardAmount: '',
    sPointBenefit: '',
    otherBenefits: '',
    benefitBreakdown: item.discountRate || item.productDiscount ? { '상품 할인': item.discountRate || item.productDiscount } : {},
    benefitBasis: basis || (loginApplied ? '상세 확인 실패' : '비회원'),
    benefitDetailsText: details || '',
    benefitText: '',
    productUrl: productUrl || item.productUrl || '',
  };
}
  ...
  for (let i = 0; i < 24; i += 1) {
    ...
    await page
      .evaluate(() => {
        ...
        const moreButton = Array.from(document.querySelectorAll('button, a, [role="button"], .btn, .button')).find(
          (node) => isVisible(node) && /더\s*보기|more|상품\s*더보기|검색결과\s*더보기/i.test(clean(node.textContent)),
        );
        moreButton?.click();
      })
      .catch(() => {});
  ...
  try {
    ...
    const extractCurrentPage = () => page.evaluate(
      ({ maxResults: limit, origin }) => {
        ...
        const datasetValue = (root, keyPattern) => {
          const nodes = [root, ...Array.from(root.querySelectorAll('*'))];
          for (const node of nodes) {
            for (const [key, value] of Object.entries(node.dataset || {})) {
              if (keyPattern.test(key) && clean(value)) return clean(value);
            }
          }
          return '';
        };

        const attrValue = (root, attrPattern) => {
          const nodes = [root, ...Array.from(root.querySelectorAll('*'))];
          for (const node of nodes) {
            for (const attr of Array.from(node.attributes || [])) {
              if (attrPattern.test(attr.name) && clean(attr.value)) return clean(attr.value);
            }
          }
          return '';
        };

        const cardLines = (root) =>
        ...
        const isNoiseLine = (line) =>
          !line ||
          /(?:^\$ ?[\d,.]+|^[\d,]+원|\d+(?:\.\d+)?\s*%|장바구니|바로구매|구매하기|관심상품|쿠폰|혜택|적립|품절|일시품절|sold\s*out|상세보기|이미지|할인율)/i.test(
            line,
          );

        const lineFallbacks = (root) => cardLines(root).filter((line) => !isNoiseLine(line));
        ...
        const normalizeProductUrl = (value) => {
          ...
          try {
            ...
            const codeMatch = url.pathname.match(/\/p\/(\d+)/);
            if (codeMatch?.[1]) {
              return new URL(`/estore/kr/ko/p/${codeMatch[1]}`, origin).toString();
            }
            return url.toString();
          } catch {
            return String(value || '');
        ...
        const productCodeFromUrl = (value) => {
          const match = String(value || '').match(/\/p\/(\d+)/);
          return match?.[1] || '';
        };
        ...
        for (const card of cards) {
          ...
          const attrText = Array.from(card.querySelectorAll('*'))
            ...
            .join(' ');
          if (/XTR_BANNER_TYPE=RECMD|XTR_BANNER_ID|XTR_MODEL_ID/i.test(`${href} ${attrText}`)) continue;

          const textSku = skuFromText(clean(card.textContent));
          const dataSku =
            datasetValue(card, /sku|stock|itemCd|itemCode|goodsCd|goodsCode|erp/i) ||
            attrValue(card, /^data-.*(sku|item|goods|erp).*$/i);
          const productSku = skuFromText(`${dataSku} ${textSku}`);
          const productUrl = normalizeProductUrl(absoluteProductUrl(href, code));
          const normalizedCode = code || productCodeFromUrl(productUrl);
          const dedupeKey = normalizedCode || productUrl;
          if (!dedupeKey || seen.has(dedupeKey)) continue;
          seen.add(dedupeKey);

          const lines = lineFallbacks(card);
          const dataProductName =
            datasetValue(card, /product.*name|goods.*name|god.*name|item.*name|prod.*name/i) ||
            attrValue(card, /^data-.*(product|goods|god|item|prod).*name/i);
          const dataBrand =
            datasetValue(card, /brand.*name|brnd.*name|brand|maker/i) ||
            attrValue(card, /^data-.*(brand|brnd|maker).*$/i);
          const brandCandidate =
            firstText(card, ['.short_brand .name', '.info_brand', 'strong.info_brand', '.product_txt .tit strong', '.brand']) ||
            dataBrand;
          const brand = brandCandidate || lines.find((line) => line.length <= 40 && !/\$|원|%/.test(line)) || '';
          const productName =
            firstText(card, [
            ...
            lines.find((line) => line !== brand && line.length >= 3) ||
            '';
          ...
          items.push({
            ...
            maxBenefitPrice: guestPrice.text || normalizedSalePrice,
    ...
    const applyInputFilters = (result) => ({
      ...result,
      items: filterSearchItemsForInput(result.items || [], input),
    });

    let extracted = applyInputFilters(await extractCurrentPage());
    const mergeExtracted = (base, addition) => {
      const mergedItems = [];
      const seen = new Set();
      for (const item of [...(base.items || []), ...(addition.items || [])]) {
        const key = item.productCode || item.productSku || item.productUrl || `${item.brand}|${item.productName}`;
        if (!key || seen.has(key)) continue;
        seen.add(key);
        mergedItems.push(item);
        if (mergedItems.length >= maxResults) break;
      }
      return {
        ...base,
        ...addition,
        items: mergedItems,
        noResultMessage: base.noResultMessage || addition.noResultMessage || '',
      };
    };
    const visitedQueries = new Set([effectiveQuery.toLowerCase()]);
    ...
    if (extracted.items.length === 0 && input.inputType === 'keyword' && redirectedBrandSlug) {
      ...
      if (retryQuery && retryQuery.toLowerCase() !== query.toLowerCase()) {
        ...
        visitedQueries.add(effectiveQuery.toLowerCase());
        effectiveSearchUrl = buildSearchUrl(retryQuery);
        await gotoAndContinue(page, effectiveSearchUrl, { timeout: 25000 });
        await settleSearchPage(page, maxResults);
        extracted = mergeExtracted(extracted, applyInputFilters(await extractCurrentPage()));
      }
    }

    if (input.inputType === 'keyword' && extracted.items.length < maxResults) {
      const retryQueries = uniqueNonEmpty([input.originalQuery, ...(input.aliasVariants || [])]);
      for (const retryQuery of retryQueries) {
        const retryKey = retryQuery.toLowerCase();
        if (!retryQuery || visitedQueries.has(retryKey)) continue;
        effectiveQuery = retryQuery;
        visitedQueries.add(retryKey);
        effectiveSearchUrl = buildSearchUrl(retryQuery);
        await gotoAndContinue(page, effectiveSearchUrl, { timeout: 25000 });
        await settleSearchPage(page, maxResults);
        extracted = mergeExtracted(extracted, applyInputFilters(await extractCurrentPage()));
        if (extracted.items.length >= maxResults) break;
      }
    }
    ...
    } else if (extracted.items.length === 0 && brandFilterForInput(input.originalQuery).length > 0) {
      noResultMessage = `${input.originalQuery} 브랜드와 일치하는 검색 결과가 없습니다.`;
    }
...
async function scrapeProductBenefits(context, item, { inputQuery, inputSku, loginApplied }) {
  ...
  try {
    ...
    await page.waitForSelector('body', { timeout: 8000 }).catch(() => {});
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const blockState = await getProductPageBlockState(page);
      if (blockState.requiresAdultVerification) {
        return blockedBenefitRow(item, {
          inputQuery,
          inputSku,
          loginApplied,
          productUrl: blockState.finalUrl || productUrl,
          basis: '성인인증 필요',
          details:
            '주류 상품 상세페이지에서 19세/성인인증 화면이 표시되어 혜택 상세를 읽지 못했습니다. 열린 테스트 Chrome에서 신라면세점 로그인과 성인인증을 완료한 뒤 다시 조회하세요.',
        });
      }
      if (blockState.requiresLogin) {
        return blockedBenefitRow(item, {
          inputQuery,
          inputSku,
          loginApplied,
          ...
          basis: '로그인 필요',
          details:
            '상품 상세페이지에서 로그인 화면이 표시되어 혜택 상세를 읽지 못했습니다. 열린 테스트 Chrome에서 신라면세점에 다시 로그인한 뒤 다시 조회하세요.',
        });
      }
      if (!blockState.blockedBySecurity) break;
      await page.waitForTimeout(1500);
    }

    const finalBlockState = await getProductPageBlockState(page);
    if (finalBlockState.requiresAdultVerification) {
      return blockedBenefitRow(item, {
        inputQuery,
        inputSku,
        loginApplied,
        productUrl: finalBlockState.finalUrl || productUrl,
        basis: '성인인증 필요',
        details:
          '주류 상품 상세페이지에서 19세/성인인증 화면이 표시되어 혜택 상세를 읽지 못했습니다. 열린 테스트 Chrome에서 신라면세점 로그인과 성인인증을 완료한 뒤 다시 조회하세요.',
      });
    }
    if (finalBlockState.requiresLogin) {
      return blockedBenefitRow(item, {
        inputQuery,
        inputSku,
        loginApplied,
    ...
    const blockedBySecurity = finalBlockState.blockedBySecurity;
    ...
    const detail = await page.evaluate(
      ({ fallback, interaction }) => {
        ...
        const detailSku =
          ...
          validSku(clean(jsonLd.sku)) ||
          validSku(firstAttr(['[data-sku]'], 'data-sku')) ||
          validSku(firstAttr(['[data-productsku]'], 'data-productsku')) ||
          validSku(firstAttr(['[data-goods-code]'], 'data-goods-code')) ||
          validSku(firstAttr(['[data-goodscode]'], 'data-goodscode')) ||
          validSku(firstAttr(['[data-item-code]'], 'data-item-code'));
    ...
    if (normalizedInput.inputType === 'productUrl') {
      ...
      const row = await scrapeProductBenefits(
        context,
        {
          ...
          maxBenefitPrice: fallback.maxBenefitPrice || '',
  ...
  for (let i = 0; i < 24; i += 1) {
    ...
    await page
      .evaluate(() => {
        ...
        const moreButton = Array.from(document.querySelectorAll('button, a, [role="button"], .btn, .button')).find(
          (node) => isVisible(node) && /더\s*보기|more|상품\s*더보기|검색결과\s*더보기|LOAD\s*MORE/i.test(clean(node.textContent)),
        );
        moreButton?.click();
      })
      .catch(() => {});
...
function normalizeShillaCompareItem(item, sourceQuery) {
  ...
  return {
    ...
    productSku: sku,
    salePrice: item.salePrice || '',
    discountPrice: item.maxBenefitPrice || item.guestPrice || item.salePrice || '',
    maxBenefitPrice: '',
    benefitBreakdown: item.discountRate || item.productDiscount ? { '상품 할인': item.discountRate || item.productDiscount } : {},
...
async function scrapeShillaCompare(keyword, maxResults) {
  try {
    const result = await scrapeBenefitRows(keyword, maxResults);
    return {
      ...result,
      items: (result.items || []).map((item) => ({
        ...
        site: '신라',
        sourceQuery: keyword,
        brandCode: item.brandCode || brandCodeFromSku(item.productSku),
        productSku: normalizeSkuCandidate(item.productSku),
        productCode: item.productCode || '',
...
async function scrapeLotteSearchItems(context, keyword, maxResults) {
  ...
  const appendCollectedItems = (items = []) => {
    for (const item of items) {
      const key = item.productCode || item.productSku || item.productUrl || `${item.brand}|${item.productName}`;
  ...
  for (const query of queries) {
    ...
    try {
        ...
        ({ origin, limit, sourceQuery }) => {
          ...
          for (const card of cards) {
            const link = card.querySelector(
              '.unit_link[data-prdno], .unit_link[data-prdNo], a[href*="/product/productDetail"], a[onclick*="productDetail"]',
            );
            ...
            let hrefUrl = null;
            try {
              hrefUrl = href ? new URL(href, origin) : null;
            } catch {
              hrefUrl = null;
            }
            const prdNo =
              link?.getAttribute('data-prdNo') ||
              link?.getAttribute('data-prdno') ||
              hrefUrl?.searchParams.get('prdNo') ||
              params.find((param) => /^\d{8,}$/.test(param)) ||
              '';
            const prdOptNo =
              link?.getAttribute('data-prdOptNo') ||
              link?.getAttribute('data-prdoptno') ||
              hrefUrl?.searchParams.get('prdOptNo') ||
              params.find((param) => /^\d{8,}$/.test(param) && param !== prdNo) ||
              prdNo;
            ...
            items.push({
              ...
              prdOptNo,
              salePrice,
              discountPrice: [maxBenefitPrice, krwPrice].filter(Boolean).join(' / '),
              maxBenefitPrice: '',
              ...
              benefitDetailsText: '',
              productUrl:
                hrefUrl && hrefUrl.pathname.includes('/product/productDetail')
                  ? hrefUrl.toString()
                  : `${origin}/kr/product/productDetail?prdNo=${encodeURIComponent(prdNo)}${
      ...
      appendCollectedItems(filterSearchItemsForInput(extracted.items, input));
      ...
      lastResult = {
        ...
        items: collectedItems.slice(0, maxResults),
...
async function scrapeLotteProductDetail(context, item, sourceQuery) {
  ...
  try {
    ...
    const couponClicks = await clickDetailControls(
      page,
      '오늘의\\s*혜택\\s*받기|쿠폰\\s*(받기|다운|다운로드)|다운로드\\s*쿠폰|쿠폰\\s*다운로드',
      { limit: 4 },
    );
    if (couponClicks.length) {
    ...
    const detail = await page.evaluate(
      ({ fallback, couponClicks: clickedCoupons }) => {
        const clean = (value) => String(value || '').replace(/\s+/g, ' ').trim();
        const bodyText = document.body?.innerText || '';
        ...
        const lines = bodyText
          ...
          .map(clean)
          .filter(Boolean);
        const hasProductText = /(상품코드|레퍼런스\s*코드|최대\s*혜택가|장바구니|바로구매|상품상세)/i.test(bodyText);
        const isLoginPage =
          /\/member\/login/i.test(finalUrl) ||
          (/(아이디\/이메일|비밀번호|회원\s*로그인|로그인)/i.test(bodyText) && !hasProductText);
        const firstText = (selectors) => {
          for (const selector of selectors) {
            const text = clean(document.querySelector(selector)?.textContent);
            if (text) return text;
        ...
        const cleanProductName = (name) => {
          let text = clean(name);
          const brandText = clean(fallback.brandEnglish || fallback.brand || '');
          if (brandText && text.includes(brandText)) {
            text = clean(text.replace(new RegExp(`^.*?${escapeRegExp(brandText)}\\s*`, 'i'), ''));
          } else if (brandText && text.toLowerCase().startsWith(brandText.toLowerCase())) {
            text = clean(text.replace(new RegExp(`^${escapeRegExp(brandText)}\\s*`, 'i'), ''));
          }
          return text;
        };
        ...
        if (isLoginPage) {
          ...
          if ((clickedCoupons || []).length) benefitBreakdown['쿠폰 다운로드'] = '로그인 필요';
          return {
            brand: fallback.brand || '',
            productName: fallback.productName || '',
            productSku: fallback.productSku || '',
            ...
            maxBenefitPrice: '',
            productDiscount: fallback.productDiscount || '',
            benefitBreakdown,
            benefitBasis: '롯데 로그인 필요',
            benefitDetailsText: '상세페이지 또는 쿠폰 받기 시도가 로그인 화면으로 이동했습니다.',
            finalUrl,
          };
        }
        ...
        const maxBenefitPrice = valueIn(maxBlock, moneyPattern) || '';
        ...
        if (fallback.productDiscount) benefitBreakdown['상품 할인'] = fallback.productDiscount;
        if ((clickedCoupons || []).length) {
          addBenefit(benefitBreakdown, '쿠폰 다운로드', clickedCoupons.join(' | '));
        }
        ...
        const benefitDetailsText = lines
          ...
          .slice(0, 70)
          .join(' | ');
        const membershipLine = lines.find((line) => /(로그아웃|회원등급|GOLD|SILVER|BLACK|VIP)/i.test(line)) || '';
        ...
        return {
          ...
          benefitBreakdown,
          benefitBasis: membershipLine ? `롯데 로그인(${membershipLine})` : '롯데 비로그인',
          benefitDetailsText,
          finalUrl,
        };
      },
      { fallback: item, couponClicks },
    );
    ...
    return {
      ...
      maxBenefitPrice: detail.maxBenefitPrice || item.maxBenefitPrice || '',
      productUrl: /^https:\/\/kor\.lottedfs\.com\/kr\/product\/productDetail/i.test(detail.finalUrl || '') ? detail.finalUrl : productUrl,
    };
  } finally {
    await page.close().catch(() => {});
  }
      ...
      const extracted = await page.evaluate(
        ({ origin, limit, sourceQuery }) => {
          ...
          for (const card of cards) {
            ...
            const discountPrice = clean(card.querySelector('.saleDollar')?.textContent || '');
            const brand = brandKo || brandEn || clean(card.querySelector('.brandName')?.textContent || '');
            if (brandEn && productName.includes(brandEn)) {
              productName = clean(productName.replace(new RegExp(`^.*?${escapeRegExp(brandEn)}\\s*`, 'i'), ''));
            } else if (brand && productName.toLowerCase().startsWith(brand.toLowerCase())) {
              productName = clean(productName.replace(new RegExp(`^${escapeRegExp(brand)}\\s*`, 'i'), ''));
            }
...
async function scrapeSsgProductDetail(context, item, sourceQuery) {
  ...
  try {
    ...
    const couponClicks = await clickDetailControls(
      page,
      '쿠폰\\s*(받기|다운|다운로드)|다운로드\\s*쿠폰|쿠폰\\s*다운로드|혜택\\s*받기|받을\\s*수\\s*있는\\s*쿠폰',
      { limit: 5 },
    );
    if (couponClicks.length) {
      await page.waitForTimeout(900);
    ...
    const detail = await page.evaluate(
      ({ fallback, couponClicks: clickedCoupons }) => {
        ...
        const lines = bodyText
          ...
          .map(clean)
          .filter(Boolean);
        const isReviewLine = (line) => /구매후기|신고|공유하기|정말\s*좋아요|[A-Z]{2,}\*{2,}/i.test(line);
        const firstBenefitIndex = lines.findIndex((line) => /(혜택|할인가|최대\s*혜택|결제혜택|쿠폰|적립|포인트|Pay|카드)/i.test(line));
        const reviewIndex = lines.findIndex(
          (line, index) => index > Math.max(firstBenefitIndex, 0) + 3 && /상품평|리뷰|구매후기|신고|공유하기|정말\s*좋아요|[A-Z]{2,}\*{2,}/i.test(line),
        );
        const benefitLines = (reviewIndex >= 0 ? lines.slice(0, reviewIndex) : lines).filter((line) => !isReviewLine(line));
        const firstText = (selectors) => {
          for (const selector of selectors) {
            const text = clean(document.querySelector(selector)?.textContent);
            if (text) return text;
        ...
        const valueNear = (matcher, pattern, size = 8) => {
          const index = benefitLines.findIndex((line) => matcher.test(line));
          if (index < 0) return '';
          return clean(benefitLines.slice(index, index + size).join(' ').match(pattern)?.[0] || '');
        };
        const moneyPattern = /\$ ?[\d,.]+|[\d,]+원/i;
        ...
        for (let i = 0; i < benefitLines.length; i += 1) {
          ...
          for (const [label, matcher] of labelMatchers) {
            if (!matcher.test(line)) continue;
            const block = benefitLines.slice(i, i + 4);
        ...
        const benefitDetailsText = benefitLines
          .filter((line) => /(혜택|할인|쿠폰|적립|포인트|Pay|카드|최대혜택가|보유한)/i.test(line))
          .slice(0, 90)
          .join(' | ');
        const requiresLoginText = /로그인\s*후|로그인이\s*필요|로그인하세요|로그인\s*필요|로그인해\s*주세요/i.test(bodyText);
        const membershipLine = lines.find((line) => /(로그아웃|회원등급|BLACK|GOLD|SILVER|VIP|\S+님)/i.test(line)) || '';
        const isLoggedIn = Boolean(membershipLine && !requiresLoginText);
        const discountPrice =
          valueNear(/할인가|할인\s*가/i, moneyPattern, 6) ||
          fallback.discountPrice ||
          '';
        const maxBenefitPrice =
          valueNear(/최대\s*혜택가|예상\s*결제|총\s*혜택가/i, moneyPattern, 10) ||
          '';
        ...
        return {
          brand:
            ...
            '',
          productName:
            cleanProductName(firstText(['.prodName, .goosNm, .productName, .product_name, h1, h2']) || fallback.productName) ||
            fallback.productName ||
            '',
          productSku: sku,
          ...
          productDiscount: fallback.productDiscount || benefitBreakdown['상품 할인'] || '',
          benefitBreakdown,
          benefitBasis: isLoggedIn ? `신세계 로그인(${membershipLine})` : requiresLoginText ? '신세계 로그인 필요' : '신세계 비로그인',
          benefitDetailsText,
          finalUrl: window.location.href,
        };
      },
    ...
    return {
      ...
      discountPrice: detail.discountPrice || item.discountPrice || '',
      maxBenefitPrice: detail.maxBenefitPrice || item.maxBenefitPrice || '',
      productUrl: /^https:\/\/www\.ssgdfs\.com\/kr\/goos\/view\//i.test(detail.finalUrl || '') ? detail.finalUrl : productUrl,
    };
  } finally {
    await page.close().catch(() => {});
  }
...
async function scrapeDutyFreeCompare(keyword, maxResults = DEFAULT_BENEFIT_MAX_RESULTS) {
  ...
  try {
    ...
    const brandFilteredResults = filterSearchItemsForInput(results, input);
    const loggedResults = brandFilteredResults.filter(isLoggedCompareRow);
    const normalizedResults = loggedResults.map(normalizeCompareOutputRow);
    ...
    if (!normalizedResults.length) {
      return {
        ...
        needsLogin: true,
        error:
          '로그인된 회원 상세 결과가 확인되지 않아 조회를 중단했습니다. 앱이 새로 연 Chrome에서 신라, 롯데, 신세계에 모두 로그인한 뒤 다시 조회해 주세요.',
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (requestUrl.pathname === '/api/benefits') {
      ...
      const sessionInfo = await getLoginSessionInfo({ probe: false });
    ...
    if (requestUrl.pathname === '/api/compare') {
      ...
      const sessionInfo = await getLoginSessionInfo({ probe: false });
      if (!sessionInfo.loginTokenValid) {
        jsonResponse(res, 428, {
          error:
            '로그인 세션이 확인되지 않아 조회하지 않습니다. 앱이 새로 연 Chrome에서 신라, 롯데, 신세계에 모두 로그인한 뒤 다시 조회해 주세요.',
          ...sessionInfo,
          needsLogin: true,
        });
        return;
      ...
      if (result.needsLogin) {
        jsonResponse(res, 428, publicResult(result));
        return;
      }
